...
%
\begin{proposition}
	Let $X$ an Euclidean shape such that the contour of $X$, denoted $\partial X$, is twice differentiable. If we evolve $X$ according to the balance coefficient flow using $r$ as the radius of the estimation disk, the contour of $X$ evolves according to
	%
	\begin{align*}
		C(s,0) &= \partial X^{(0)} \\	
		C(s,t) &= \partial X^{(t)} \\
		&= \frac{1}{6} \frac{\partial ^2 C}{\partial s^2} r^2 + O(r^4) .
	\end{align*}
	%
