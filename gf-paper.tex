...
%
%
We are going to extend equation~\ref{eq:curvature_approximation} to the continuous domain. We call this continuous extended version of equation~\ref{eq:curvature_approximation} of \emph{balance coefficient}
%
%
\begin{align*}
...
%
%
 We interpret the continuous domain as a digitization with a infinitely small grid step. Therefore, for points closer to the contour of $S$, the balance coefficient at $P$ gives us as an \emph{approximation} of the new curvature value were the shape perturbed a little around $P$. Our strategy to decrease the elastica energy is to evolve the shape towards the zero-level set of the balance coefficient function (see figure~\ref{fig:balance-coefficient-zero-level-set}). 

\begin{definition}[Balance coefficient flow]
	Let $X \subset \Omega$ an Euclidean shape and $r$ the radius of the estimation disk used to compute the balance coefficient. We define the \emph{balance coefficient flow} on the discrete time parameter $\tau$ as
%
%
\begin{align*}
	X^{(0)} &= X, \\
	X^{(\tau+1)} &= \left\{ P \in \Omega \: | \: u_r(X^{(\tau)},P) \leq 0 \right\}.
\end{align*}
%
%
\end{definition}
...
\caption{\textbf{Geometric interpretation of the balance coefficient flow.} A single iteration of the balance coefficient flow evolves the shape $X$ to the shape $X'$ such that any disk of radius $r$ centered at any point of the contour of $X'$ intersects  an area of $\pi r^2/2$ of initial shape $X$.}
\label{fig:geometric-interpretation}
\end{figure}
% 
%
 Additionally, the balance coefficient flow is closely related with the curve-shortening flow (CSF is the one-dimensional case of the mean curvature flow).
%
%
\begin{figure}
\center
\includegraphics[scale=0.75]{figures/analysis-error/geometry-argument.png}
\caption{\textbf{Balance coefficient and curve-shortening flow (CSF).} We approximate the contour $\partial X$ at $P$ by a parabola. Next, we compute the point $P'$ such that the balance coefficient equals to zero. Our approximation produces an error $\Delta$, highlighted in red, that is of order $O(r^5)$. }
\label{fig:geometric-argument}
\end{figure}
%
%
\begin{proposition}
	Let $X$ an Euclidean shape such that the contour of $X$, denoted $\partial X$, is twice differentiable and parameterizable by arc length. If we evolve $X$ according to the balance coefficient flow using $r$ as the radius of the estimation disk, the contour of $X$ evolves according to
	%
	\begin{align*}
		C(s,0) &= \partial X^{(0)} \\	
		C(s,\tau + 1) &= \partial X^{(\tau + 1)} \\
		&= \frac{1}{6} \frac{\partial ^2 C(\tau)}{\partial s^2} r^2 + O(r^5) .
	\end{align*}
	%
...
<<<<<<< HEAD
	\Leftrightarrow & \varepsilon &= \frac{1}{6}\kappa r^2.
\end{array}
\]
%
%
We proceed by computing an upper bound of the error $\Delta$. Let $A^3$ be the intersection point between the line $A'B'$ and the estimation disk that is the closest from $A'$. It is clear that $\Delta/2$ is smaller than the region $A''A'A^3$ and the arc going from $A^3$ to $A''$.
%
%
%
%\begin{align}
...
TO BE FINISHED.
=======
	\Leftrightarrow & \varepsilon &= \frac{1}{6}\kappa r^2 \pm \frac{\Delta}{2}.
\end{array}
\]
%
%
...
%
\begin{align*}
	\varepsilon &= \frac{1}{6}\kappa r^2 \pm \frac{1}{48}\kappa^3r^5.
\end{align*}
%
%
>>>>>>> be3c9703d1390033900243e47185608c9bd34f65
\end{proof}
%
%
Proposition~\ref{prop:balance-coefficient-curve-shortening-flow} states that, if the radius of the estimation disk is sufficiently small, then the balance coefficient flow behaves similar to the curve-shortening flow.
...
%
%
In practice, we are going to show that we can achieve good results without being too strict regardind the restriction above.
%
%
