...
The flow in figure~\ref{} is quite similar to the curve-shortening flow~\cite{grayson1987heat,ecker2008heat}. This flow is commonly described as the flow that decreases the curve length the quickest. In fact, under the curve shortening flow, one can show that the length varies according to
%
\begin{align*}
...
Indeed, we have some evidence that $S{(t)}$ approximates the curve-shortening flow. We start by giving an ingenious geometric reason for that, first enounced in~\cite{merriman1992diffusion}.
